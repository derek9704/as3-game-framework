...
function initDecodeJson():Function
{
	...
	// \/ -> /
	charConvert[0x62] = 0x8;
	// \b -> backspace
	charConvert[0x66] = 0xc;
	// \f -> formfeed
	charConvert[0x6e] = 0xa;
	// \n -> newline
	charConvert[0x72] = 0xd;
	// \r -> carriage return
	charConvert[0x74] = 0x9;
	// \t -> horizontal tab

	const isNumberChar:ByteArray = new ByteArray();
	isNumberChar.length = 0x100;
	// fill w/ 0's
	isNumberChar[0x2b] = 1;
	// +
	isNumberChar[0x2d] = 1;
	...
	const parseStringEscaped:Function = function(result:String):String
	{
		do
		{
			...
			if ((char = byteInput[position++]) === 0x75)
			{
				// \uxxxx -> utf8 char
	...
	const parse:Object = {0x22:function():String
	{
		// "
	...
	}, 0x5b:function():Object
	{
		...
		return result;
	}, 0x74:function():Boolean
	{
		// t
		if (byteInput[position] === 0x72 && byteInput[position + 1] === 0x75 && byteInput[position + 2] === 0x65)
		{
			// == rue
			position += 3;
			return true;
	...
	}, 0x66:function():Boolean
	{
		// f
		if (byteInput[position] === 0x61 && byteInput[position + 1] === 0x6c && byteInput[position + 2] === 0x73 && byteInput[position + 3] === 0x65)
		{
			// == alse
			position += 4;
			return false;
	...
	}, 0x6e:function():Object
	{
		// n
		if (byteInput[position] === 0x75 && byteInput[position + 1] === 0x6c && byteInput[position + 2] === 0x6c)
		{
			// == ull
			position += 3;
			return null;
	...
	return function(input:*):Object
	{
		...
		catch (e:TypeError)
		{
			...
			{
				e.message = "Server Error : " + input +  " (" + e.message + ")";
