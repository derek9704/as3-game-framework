...
function initDecodeJson():Function
{
	...
	for (j = 0; j < 0xa; j++)
	{
		charConvert[j] = (j + 0x30) | 0x30303000;
			// 000[0-9]
	}
	for (; j < 0x10; j++)
	{
		charConvert[j] = (j + 0x37) | 0x30303000;
			// 000[A-F]
	}
	for (; j < 0x1a; j++)
	{
		charConvert[j] = (j + 0x20) | 0x30303100;
			// 00[1][0-9]
	}
	for (; j < 0x20; j++)
	{
		charConvert[j] = (j + 0x27) | 0x30303100;
			// 00[1][A-F]
	}
	for (; j < 0x100; j++)
	{
	...
	// \n
	charConvert[0xd] = 0x5c72;
	// \r
	charConvert[0x9] = 0x5c74;
	// \t
	charConvert[0x8] = 0x5c62;
	// \b
	charConvert[0xc] = 0x5c66;
	// \f
	charConvert[0x8] = 0x5c62;
	// \b
	...
	const parse:Object = {"object":function(data:Object):void
	{
		if (data)
		{
			if (data is Array)
		...
		{
			result.writeUnsignedInt(0x6e756c6c);
				// null
		}
	}, "string":parseString, "number":function(data:Number):void
	{
	...
	}, "boolean":function(data:Boolean):void
	{
		if (data)
		{
			result.writeUnsignedInt(0x74727565);
				// true
		}
		else
		{
			result.writeByte(0x66);
			// f
			result.writeUnsignedInt(0x616c7365);
				// alse
		}
