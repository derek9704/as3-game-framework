	...
	public class JsonEncoderAsync
	{
		...
		public function JsonEncoderAsync(input:*, writeTo:IDataOutput = null, strictNumberParsing:Boolean = false):void
		{
			...
			if (!_charConvert)
			{
				...
				{
					_charConvert[j] = (j + 0x30) | 0x30303000;
						// 000[0-9]
				}
				for (; j < 0x10; j++)
				{
					_charConvert[j] = (j + 0x37) | 0x30303000;
						// 000[A-F]
				}
				for (; j < 0x1a; j++)
				{
					_charConvert[j] = (j + 0x20) | 0x30303100;
						// 00[1][0-9]
				}
				for (; j < 0x20; j++)
				{
					_charConvert[j] = (j + 0x27) | 0x30303100;
						// 00[1][A-F]
				}
				for (; j < 0x100; j++)
				{
				...
				// \n
				_charConvert[0xd] = 0x5c72;
				// \r
				_charConvert[0x9] = 0x5c74;
				// \t
				_charConvert[0x8] = 0x5c62;
				// \b
				_charConvert[0xc] = 0x5c66;
				// \f
				_charConvert[0x8] = 0x5c62;
				// \b
		...
		private function parseValue(input:*):void
		{
			switch (typeof input)
			{
				case "object":
					...
					{
						_output.writeUnsignedInt(0x6e756c6c);
							// null
					}
					return;
				case "string":
				...
				case "boolean":
					if (input)
					{
						_output.writeUnsignedInt(0x74727565);
							// true
					}
					else
					{
						_output.writeByte(0x66);
						// f
						_output.writeUnsignedInt(0x616c7365);
							// alse
					}
					return;
				case "undefined":
		...
		private function parseObject(input:Object):void
		{
			...
			if (length >= 2)
			{
				...
				{
					// process an object key after a value
					stackTop = value;
					_output.writeByte(0x2c);
					// ,
			...
			else if (length >= 1)
			{
				stackTop = function():void
				{
					// process only object key
					stackTop = function():void
					{
						// process only object value
						_output.writeByte(0x7d);
						// }
						stackTop = stackNext;
					};
