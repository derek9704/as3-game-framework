	...
	public class JsonDecoderAsync
	{
		...
		public function JsonDecoderAsync(input:*, autoSubscribe:Boolean = true):void
		{
			...
			if (!_charConvert.length)
			{
				...
				// \/ -> /
				_charConvert[0x62] = 0x8;
				// \b -> backspace
				_charConvert[0x66] = 0xc;
				// \f -> formfeed
				_charConvert[0x6e] = 0xa;
				// \n -> newline
				_charConvert[0x72] = 0xd;
				// \r -> carriage return
				_charConvert[0x74] = 0x9;
					// \t -> horizontal tab
			}
		...
		public function process(limit:uint = 0):Boolean
		{
			...
			try
			{
				mainloop:while (_stack.length > 0)
				{
					...
					{
						case -1:
							// start parsing an unknown value
						...
						case 0x22:
							...
							continue mainloop;
						case 0x200:
							// parse the rest of a escape (\x)
							if ((char = _input.readUnsignedByte()) !== 0x75)
							{
								// \uxxxx
						...
						case 0x201:
							// parse the rest of a unicode escape (\uxxxx)
						...
						case 0x7b:
							...
							else
							{
								if (char === 0x22)
								{
									...
									_stack[_stack.length] = 0x300;
									// object parser, next stage
						...
						case 0x401:
							...
							continue mainloop;
						case 0x74:
							// t (start parsing true)
							if (_input.bytesAvailable >= 3)
							{
								if (_input.readShort() === 0x7275 && _input.readUnsignedByte() === 0x65)
								{
									// == rue
									result = true;
									_stack.pop();
							...
							return false;
						case 0x66:
							// f (start parsing false)
							if (_input.bytesAvailable >= 4)
							{
								if (_input.readInt() === 0x616c7365)
								{
									// == alse
									result = false;
									_stack.pop();
							...
							return false;
						case 0x6e:
							// n (start parsing null)
							if (_input.bytesAvailable >= 3)
							{
								if (_input.readShort() === 0x756c && _input.readUnsignedByte() === 0x6c)
								{
									// == ull
									result = null;
									_stack.pop();
							...
							return false;
						case 0x100:
							// number parser when the number isn't the only value
							while ((char = _input.readUnsignedByte()) !== 0x5d && char !== 0x7d && char !== 0x2c)
							{
								// is it ",", "]", or "}"?
								_buffer.writeByte(char);
							}
							...
							if (_stack[_stack.length - 2] === 0x302)
							{
							...
							else if (_stack[_stack.length - 2] === 0x400)
							{
						...
						case 0x20:
							...
							break;
						case 0x101:
							// number parser when the number is the only value
							while (_input.bytesAvailable)
							{
								if (((char = _input.readUnsignedByte()) >= 0x30 && char <= 0x39) || char === 0x65 || char === 0x45 || char === 0x2e || char === 0x2b || char === 0x2d)
								{
									// 0-9, e, E, ., +, -
									_buffer.writeByte(char);
								}
